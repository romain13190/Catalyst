...
from grail.constants import CHALLENGE_K, PRIME_Q
from grail.protocol.crypto import indices_from_root, prf
...
class TestMinerCannotControlRandomness:
    ...
    @patch("grail.shared.forward.forward_single_layer")
    ...
    def test_verifier_ignores_commit_beacon_randomness(self, _rhs, mock_fwd):
        ...
        with patch(
            "grail.protocol.crypto.indices_from_root",
...
class TestMinimumChallengesEnforced:
    ...
    def test_too_few_checks_fails_even_if_all_pass(self, _rhs, mock_fwd):
        ...
        with patch("grail.protocol.crypto.indices_from_root", return_value=[0, 1]), \
             patch("grail.protocol.grail_verifier.GRAILVerifier.verify_commitment", return_value=(True, {})):
    ...
    def test_full_checks_can_pass(self, _rhs, mock_fwd):
        ...
        with patch("grail.protocol.crypto.indices_from_root", return_value=list(range(expected))), \
             patch("grail.protocol.grail_verifier.GRAILVerifier.verify_commitment", return_value=(True, {})):
            result, passed, checked = verify_commitment_proofs(
                commit, _make_mock_model(), VALIDATOR_RANDOMNESS
            )
    ...
    def test_one_failure_rejects(self, _rhs, mock_fwd):
        ...
        with patch("grail.protocol.crypto.indices_from_root", return_value=list(range(expected))), \
             patch("grail.protocol.grail_verifier.GRAILVerifier.verify_commitment", side_effect=verify_side):
            result, passed, checked = verify_commitment_proofs(
                commit, _make_mock_model(), VALIDATOR_RANDOMNESS
            )
...
class TestStorageHmacIntegrity:
    ...
    def test_hmac_deterministic(self):
        from grail.infrastructure.storage import _compute_state_hmac
    ...
    def test_key_derivation_uses_domain_separator(self):
        from grail.infrastructure.storage import _state_hmac_key
        expected = hashlib.sha256(b"grail-state-hmac|" + b"known").digest()
...
class TestValidatorStateIsolation:
    ...
    def test_load_accepts_validator_id(self):
        from grail.infrastructure.storage import load_used_indices
        assert "validator_id" in inspect.signature(load_used_indices).parameters

    def test_save_accepts_validator_id(self):
        from grail.infrastructure.storage import save_used_indices
...
class TestSketchForgeryResistance:
    ...
    def test_one_corrupted_position_rejects_proof(self):
        ...
        with patch("grail.shared.forward.forward_single_layer", return_value=(hidden, None)), \
             patch("grail.shared.hf_compat.resolve_hidden_size", return_value=HIDDEN_DIM):
            result, passed, checked = verify_commitment_proofs(
