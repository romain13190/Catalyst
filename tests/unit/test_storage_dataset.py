...
from grail.infrastructure.storage import upload_window_dataset
...
async def test_upload_window_dataset_writes_gzipped_json_at_expected_key() -> None:
    ...
    with patch("grail.infrastructure.storage.get_s3_client", _fake_get_client):
        ok = await upload_window_dataset(1000, data, bucket_name="testbucket")

    assert ok is True
    assert captured["bucket"] == "testbucket"
    assert captured["key"] == "grail/dataset/window-1000.json.gz"
