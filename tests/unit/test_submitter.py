...
from grail.constants import COMPLETIONS_PER_SUBMISSION, VALIDATOR_HTTP_PORT
...
async def test_submit_batch_retries_on_5xx_then_succeeds() -> None:
    ...
    transport = httpx.MockTransport(handler)
    with patch("grail.miner.submitter.asyncio.sleep") as fake_sleep:
...
async def test_submit_batch_raises_after_all_5xx_attempts() -> None:
    ...
    transport = httpx.MockTransport(handler)
    with patch("grail.miner.submitter.asyncio.sleep", return_value=None):
        async with httpx.AsyncClient(transport=transport) as client:
