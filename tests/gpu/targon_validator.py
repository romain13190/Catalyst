"""Deploy a GRAIL validator on a Targon H100.
...
Usage:
    targon deploy tests/gpu/targon_validator.py --name grail-validator
"""
...
ENV_VARS = {
    ...
    "R2_BUCKET_ID": os.environ.get("R2_BUCKET_ID", "grail-catalyst-test"),
    "GRAIL_STATE_HMAC_KEY": os.environ.get(
        "GRAIL_STATE_HMAC_KEY", "catalyst-local-test-hmac"
    ),
}
...
app = targon.App("grail-validator", image=image)
...
def serve():
    import subprocess
    cmd = [
        "python", "-m", "grail.cli.main", "validate",
        "--network", NETWORK,
        "--netuid", str(NETUID),
